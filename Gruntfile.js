...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    compress: {
      ...
      finalWindowsApp: {
        options: {
          archive: '<%= config.dist %>/TraktTvStatus.zip'
        },
        files: [{
          expand: true,
          cwd: '<%= config.tmp %>',
    ...
    rename: {
      app: {
        files: [{
          ...
          dest: '<%= config.dist %>/TraktTvStatus.app'
        }]
      },
      zipToApp: {
        files: [{
  ...
  grunt.registerTask('chmod', 'Add lost Permissions.', function () {
    var fs = require('fs');
    fs.chmodSync('dist/TraktTvStatus.app/Contents/Frameworks/node-webkit Helper EH.app/Contents/MacOS/node-webkit Helper EH', '555');
    fs.chmodSync('dist/TraktTvStatus.app/Contents/Frameworks/node-webkit Helper NP.app/Contents/MacOS/node-webkit Helper NP', '555');
    fs.chmodSync('dist/TraktTvStatus.app/Contents/Frameworks/node-webkit Helper.app/Contents/MacOS/node-webkit Helper', '555');
    fs.chmodSync('dist/TraktTvStatus.app/Contents/MacOS/node-webkit', '555');
  });
  ...
  grunt.registerTask('createWindowsApp', 'Create windows distribution.', function () {
    ...
    concat([
      'tmp/nw.exe',
      'tmp/app.nw',
    ], 'tmp/TraktTvStatus.exe', function () {
      var fs = require('fs');
  ...
  grunt.registerTask('dmg', 'Create dmg from previously created app folder in dist.', function () {
    var done = this.async();
    var createDmgCommand = 'resources/mac/package.sh "TraktTvStatus"';
